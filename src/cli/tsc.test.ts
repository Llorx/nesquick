...
test.describe("tsc", test => {
    ...
    const transformer: TS.TransformerFactory<TS.SourceFile> = context => {
        return sourceFile => {
            const visitor = (node: TS.Node): TS.Node => {
                console.log(" - node", SyntaxKind[node.kind]);
                try {
    ...
    /*test("should compile project", {
        ...
        ASSERTS: {
            ...
            async "files should be created"(_, { mockPath }) {
                return Fs.promises.stat(Path.join(mockPath, "lib", "required.js"));
            }
        }
    });*/
    test("should apply transforms", {
        async ARRANGE(after) {
            const mockPath = Path.resolve(__dirname, "..", "..", "veact-tsc-mock");
            const libPath = after(Path.join(mockPath, "lib"), clearPath);
            await clearPath(libPath);
